...
public:
	...
	void compute_error(const std::string &filename, int fr, std::vector<float> &points, std::vector<float> &reflect, std::vector<int> &in_points_per_pix,
					   std::vector<float> &background, float time, float distance = 40, float hyperparam = 0);

	void save_timing(const std::string &filename, float time);
