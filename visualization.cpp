...
void visualization::showPointCloud(void) {
	...
	if (data_available) {
		...
		for (int i = 0; i < clouds[0]->size(); i++) {
			...
			p.r = color.r; p.b = color.b; p.g = color.g;
			color_cloud->at(i) = p;
		}
...
visualization::rgb visualization::map_reflectivity_to_color(std::vector<float> & r, float  lim_max, float  lim_min) {
	...
	if (wavelengths == 1) {
		unsigned char c = unsigned char(255.*(r[0] - lim_min) / (lim_max - lim_min));
		color = colormap(c);
	}else {
		color.b = unsigned char(255.*(r[0] - lim_min) / (lim_max - lim_min));
		color.g = unsigned char(255.*(r[1] - lim_min) / (lim_max - lim_min));
		color.r = unsigned char(255.*(r[3] - lim_min) / (lim_max - lim_min));
	}
...
int visualization::loadPointCloud(std::vector<float> & points, std::vector<float> & normals, std::vector<float> &  reflect, std::vector<int> &  in_points_per_pix,
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			...
			img_points_per_pix[j + i * width] = colormap(unsigned char(32* number_points));
			...
			for (int idx = 0; idx < number_points; idx++) {
				...
				point.normal[0] = p.normal()(2); point.normal[1] = -p.normal()(1); point.normal[2] = -p.normal()(0);

				point.x = p.pos()(2); point.y = width -p.pos()(1); point.z = height - p.pos()(0);
				point.z *= scale_ratio;	point.y *= scale_ratio;	point.x *= scale_ratio;
...
void visualization::savePly(std::string & filename) {
	bool binary = true;
	if (data_available) {
