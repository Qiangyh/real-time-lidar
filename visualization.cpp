...
visualization::rgb visualization::map_reflectivity_to_color(std::vector<float> &r, float lim_max, float lim_min)
{
	...
	{
		color.b = (unsigned char)(255.0 * (r[0] - lim_min) / (lim_max - lim_min));
		color.g = (unsigned char)(255.0 * (r[1] - lim_min) / (lim_max - lim_min));
		color.r = (unsigned char)(255.0 * (r[3] - lim_min) / (lim_max - lim_min));
	}
