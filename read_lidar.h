...
public:
	...
	LidarData(int sketches = 0)
	{ // if sketches = 0, no sketching assumed
	...
	{
		if (data_type == SPARSE)
			return int(sparse_frames.size());
		else if (data_type == DENSE)
			return int(dense_frames.size());
		else
			return int(sketched_frames.size());
	};

	size_t getTotalActive(int fr = 0)
	{
		if (data_type == SPARSE)
			return sparse_frames[fr].data().size();
		else
			return size_t(T);
	};
	...
	photon *getAllBinsCounts(int fr = 0)
	{
		if (data_type == SPARSE)
			return &(sparse_frames[fr].data()[0]);
		else
			return NULL;
	};

	int *getDense(int fr = 0)
	{
		if (data_type == DENSE)
			return &(dense_frames[fr].data()[0]);
		else
			return NULL;
	};

	float *getSketched(int fr = 0)
	{
		if (data_type == SKETCHED)
			return &(sketched_frames[fr].data()[0]);
		else
			return NULL;
	};

	int *getPerPixActiveBins(int fr = 0)
	{
		if (data_type == SPARSE)
			return &(sparse_frames[fr].bins_act()[0]);
		else
			return NULL;
	};

	int *getPerPixActiveBinsIdx(int fr = 0)
	{
		if (data_type == SPARSE)
			return &(sparse_frames[fr].bins_act_idx()[0]);
		else
			return NULL;
	};
	...
	{
		if (data_type == SPARSE)
			return sparse_frames[fr].getPPP();
		else if (data_type == DENSE)
			return dense_frames[fr].getPPP();
		else
			return sketched_frames[fr].getPPP();
	};
...
private:
	...
	file_list getFileList(file_types);
